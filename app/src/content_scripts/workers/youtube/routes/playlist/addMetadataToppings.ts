...
const addMetadataToppings = async (playlistID: string): Promise<void> => {
  ...
  let MetadataToppings = document.querySelector(
  ...
  if (MetadataToppings === null) {
    ...
    if (playlistID === "WL" || playlistID === "LL") {
      ...
      await fetchYouTubeToppings<YouTubePlaylistMetadata>({
        ...
        const averageRuntimeValueElement = document
          .getElementById("toppings__average-runtime")
          ?.querySelector(".toppings__item-value") as HTMLSpanElement;
        const totalRuntimeValueElement = document
          .getElementById("toppings__total-runtime")
          ?.querySelector(".toppings__item-value") as HTMLSpanElement;
        if (
          averageRuntimeValueElement !== null &&
          totalRuntimeValueElement !== null
        ) {
...
const addRuntimeSection = async (playlistID: string): Promise<HTMLElement> => {
  ...
  AverageRuntimeLabel.setAttribute(
    "class",
    "toppings__average-runtime toppings__item-label",
  );
  ...
  AverageRuntimeValue.setAttribute(
    "class",
    "toppings__average-runtime toppings__item-value",
  );
  ...
  TotalRuntimeLabel.setAttribute(
    "class",
    "toppings__total-runtime toppings__item-label",
  );
  TotalRuntimeLabel.appendChild(document.createTextNode("Total Runtime: "));

  const TotalRuntimeValue = document.createElement("span");
  TotalRuntimeValue.setAttribute(
    "class",
    "toppings__total-runtime toppings__item-value",
  );
