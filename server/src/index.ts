...
router.get('/playlist/:playlistID', async (request: IRequest, env: Env) => {
	...
	try {
		const data = await getPlaylistStats(playlistID, env.GOOGLE_SECRET);
...
const getPlaylistStats = async (playlistID: string, apiKey: string) => {
	let videoCount = 0;
	let totalRuntime = 0;
	let nextPageToken = '';
